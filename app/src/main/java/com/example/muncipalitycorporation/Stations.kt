...
class Stations : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        mDatabase = FirebaseDatabase.getInstance().getReference("Regional Representation").child("Stations")
        mRecyclerView = findViewById(R.id.recycler_view_stations)
        ...
        )
        {
            override fun populateViewHolder(
                ...
                viewHolder.mView.hloc.setOnClickListener {
                    val gmmIntentUri =
                            Uri.parse("geo: 22.693616, 72.857251?q=${model?.name},${model?.loc}")
